...
class TrainingConfig:
    ...
    # BTC-pulse focus: steer Grok's bounded advisory budget onto real BTC/ETH directional
    # ("Up or Down" / short-horizon) markets FIRST (the lane we trade), instead of spending it
    # on arbitrary near-misses / liquidity targets. ``strict`` restricts Grok to ONLY research
    # BTC/ETH markets (no fallback) so 100% of the budget covers the pulse universe.
    grok_btc_focus_enabled: bool = False
    grok_btc_focus_strict: bool = False
    ...
    def aggressive_paper(cls, **overrides) -> "TrainingConfig":
        ...
        * bounded **controlled exploration** of near-miss candidates at a small
        ...
        base = dict(
            ...
            grok_btc_focus_enabled=_envb("GROK_BTC_FOCUS_ENABLED", True),
            grok_btc_focus_strict=_envb("GROK_BTC_FOCUS_STRICT", False),
