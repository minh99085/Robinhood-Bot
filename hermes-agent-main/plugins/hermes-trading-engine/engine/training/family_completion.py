...
def expand_event_families(records: list, *, now: Optional[float] = None,
                          ...
                          min_family_liquidity_usd: float = 0.0) -> "tuple[list, dict]":
    """Append authoritative missing sibling records for event families present in
    ``records``. Returns ``(records_out, telemetry)``. Pure aside from reading the
    records' own ``raw`` payloads; never raises on a single malformed family."""
    from engine.markets.universe_manager import MarketRecord

    records = list(records or [])
    existing_ids = {str(getattr(r, "market_id", "") or "") for r in records}
    # group scanned records by event family key (only families with embedded sibling lists)
    ...
    for rec in records:
        ...
        if len(sibs) < 2:                       # nothing to complete from a lone market
            continue
