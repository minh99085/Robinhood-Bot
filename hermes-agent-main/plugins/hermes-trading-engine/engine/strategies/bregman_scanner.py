...
class BregmanPaperScanner:
    ...
    def scan(self, markets: Sequence[dict], *, now: Optional[float] = None) -> dict:
        ...
        tel = {
            ...
            # explicit per-stage rejection taxonomy: adapter_failed (discovery) vs
            # certifier_no_positive_profit vs realism_fees_spread_depth.
            "stage_rejections": {"adapter_failed": len(skipped),
                                 **diag.get("stage_rejections", {})},
            # projected-profit / Bregman-distance proof (logged even when 0 certified)
            "best_projected_profit_per_set": diag.get("best_projected_profit_per_set", 0.0),
            "max_bregman_distance": diag.get("max_bregman_distance", 0.0),
            "mean_cost_per_set": diag.get("mean_cost_per_set", 0.0),
            "near_miss_certified_samples": diag.get("near_miss_certified_samples", []),
            "near_miss_count": diag.get("near_miss_count", 0),
    ...
    def _disabled_telemetry(self, markets_seen: int) -> dict:
        return {
            ...
            "mean_cost_per_set": 0.0,
            "near_miss_certified_samples": [],
            "near_miss_count": 0,
