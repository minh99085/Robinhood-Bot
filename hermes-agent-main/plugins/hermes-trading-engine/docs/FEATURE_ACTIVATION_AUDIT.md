...
## Summary
- **Truly active (control trades):** Trainer Bregman certifier, Bregman paper execution, Bregman INPUT UNIVERSE (catalog vs shortlist), Profitability-first ranking, Random/hash exploration, Stale-book rejection, Spread/depth gates, Ambiguity gate, Chainlink conditioning, News/research/model overlay, Profitability governor, Position/open-slot governor, Stop-loss/take-profit/settlement handling
- **Telemetry-only:** Raw ABCAS/Bregman scanner, Grok/LLM reasoning overlay
- **Dead / imported-only:** Graph grouping (groups_from_graph), Active learning selector, Reference-price fill fallback
